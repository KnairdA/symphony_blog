...
<xsl:template name="format-weekday">
	...
	<xsl:choose>
		<xsl:when test="$weekday = 1">Montag</xsl:when>
		<xsl:when test="$weekday = 2">Dienstag</xsl:when>
		<xsl:when test="$weekday = 3">Mittwoch</xsl:when>
		<xsl:when test="$weekday = 4">Donnerstag</xsl:when>
		<xsl:when test="$weekday = 5">Freitag</xsl:when>
		<xsl:when test="$weekday = 6">Samstag</xsl:when>
		<xsl:when test="$weekday = 7">Sonntag</xsl:when>
	</xsl:choose>
	</xsl:param>
	<xsl:choose>
